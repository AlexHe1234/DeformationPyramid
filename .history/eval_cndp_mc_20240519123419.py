...
if __name__ == "__main__":
    ...
    for i in tqdm(range( len(D))):
        ...
        ate, d01, d02 = eval_metric(torch.stack(traj_pred), batch['tracks'])

        eval_dict['ate'].append(ate)
        eval_dict['0.1'].append(d01)
        eval_dict['0.2'].append(d02)
        eval_dict['time'].append((end_time - start_time) / traj_pred.shape[0])

        print(f'ate {ate} d01 {d01} d02 {d02} time {end_time - start_time}')
